...
import com.proxy.proxyee.crt.CertPool;
...
import com.proxy.proxyee.proxy.ProxyConfig;
import com.proxy.proxyee.proxy.ProxyHandleFactory;
import com.proxy.proxyee.server.HttpProxyServer;
import com.proxy.proxyee.server.HttpProxyServerConfig;
import com.proxy.proxyee.server.auth.HttpAuthContext;
import com.proxy.proxyee.server.auth.HttpProxyAuthenticationProvider;
import com.proxy.proxyee.server.auth.model.HttpToken;
import com.proxy.proxyee.util.ProtoUtil;
import com.proxy.proxyee.util.ProtoUtil.RequestProto;
...
public class HttpProxyServerHandler extends ChannelInboundHandlerAdapter {
    ...
    private void release(Object msg) {
        if (msg instanceof ByteBuf) {
            ...
            log.debug("资源释放：{} {}", i, byteBuf.refCnt());
        }

        if (msg instanceof HttpContent) {

            HttpContent httpContent = (HttpContent) msg;
            int i = httpContent.refCnt();
            if (i > 0) {
                httpContent.release();
            }
            log.debug("资源释放：{} {}", i, httpContent.refCnt());
        }
    }
}
